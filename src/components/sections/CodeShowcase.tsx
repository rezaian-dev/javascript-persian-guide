...
const CHIPS = ["Closure", "Event Loop", "Promise", "Prototype", "ESM", "V8"];

const LINES = [
  { text: "// Closure — تابعی که محیطش را با خود می‌برد", cls: "text-faint" },
  { text: "function counter() {", cls: "text-amber-200" },
  { text: "  let count = 0;", cls: "text-amber-200" },
  { text: "  return () => ++count; // محیط lexical زنده می‌ماند", cls: "text-primary-soft" },
  { text: "}", cls: "text-amber-200" },
  { text: "", cls: "" },
  { text: "// Microtask همیشه قبل از Macrotask", cls: "text-faint" },
  { text: "queueMicrotask(() => console.log(\"promise\"));", cls: "text-amber-200" },
  { text: "setTimeout(() => console.log(\"timeout\"), 0);", cls: "text-amber-200" },
  { text: "// promise → timeout", cls: "text-faint" },
];
          ...
          <div ref={ref} className="glass grid items-center gap-10 rounded-3xl p-6 md:grid-cols-[0.85fr_1.15fr] md:p-10">
            <div>
              ...
              <h2 className="text-[clamp(26px,3.5vw,38px)] font-extrabold leading-tight">
                کد <span className="grad-text">واقعی</span>، نه اسباب‌بازی
              </h2>
              <p className="mt-3.5 text-[15px] leading-8 text-muted-foreground">
                هر الگو با کد کامل و قابل اجرا می‌آید؛ ۱۷۳ پنجرهٔ کد در طول کتاب.
              </p>
            ...
            <div dir="ltr" className="overflow-hidden rounded-2xl border border-input bg-card shadow-[0_40px_110px_-30px_rgb(6_10_24_/_0.65)]">
              <div className="flex items-center gap-1.5 border-b border-border bg-secondary/40 px-4 py-3 font-mono text-[11.5px] text-muted-foreground">
                ...
                <span className="ms-2">js-mindset.js</span>
              </div>
